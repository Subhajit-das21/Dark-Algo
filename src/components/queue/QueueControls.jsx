...
import { enqueue, dequeue } from '../../utils/queueAlgorithms';
...
export default function QueueControls() {
    ...
    const executeOp = (algoFn, actionType, ...args) => {
        ...
        // Immediate functional mutation mirroring to store
    ...
    return (
        <div className="flex flex-col gap-2">
            <Dropdown title="Simple Queue" defaultOpen={true}>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                    <button
                        ...
                        onClick={() => executeOp(enqueue, 'enqueue', parseInt(inputValue, 10))}
                        className="bg-white/5 border border-white/10 text-[#19ff9c] p-2 rounded hover:bg-white/10 disabled:opacity-50 transition-colors"
                    >
                        Enqueue
                    </button>
                    <button
                        disabled={isPlaying}
                        onClick={() => executeOp(dequeue, 'dequeue')}
